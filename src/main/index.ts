...
function registerIpc() {
  ...
  ipcMain.handle('groups:delete', (_, groupId: string) => { const list = getData().groups.find(g => g.id === groupId); if (!list) throw new Error('Broadcast was not found.'); getData().groups = getData().groups.filter(g => g.id !== groupId); getData().contacts.forEach(c => { c.groupIds = c.groupIds.filter(id => id !== groupId) }); scheduleSave(); return list.name })
